...
import br.unitins.topicos1.floricultura.service.FileService;
import br.unitins.topicos1.floricultura.validation.GeneralValidationException;
import jakarta.enterprise.context.ApplicationScoped;

@ApplicationScoped
public class AlunoFileService implements FileService {

  private static final Logger LOG = Logger.getLogger(AlunoFileService.class);
  ...
  private final String PATH_USER = System.getProperty("user.home") +
    ...
    File.separator + "aluno" + File.separator;
  ...
  public String salvar(Long idSubpasta, byte[] arquivo) throws IOException {
    ...
    try {
      ...
      throw new GeneralValidationException("Imagem aluno", "Erro ao determinar a extensão do arquivo");
    }
    ...
    try {
      ...
      if (response.statusCode() >= 200 && response.statusCode() < 300) {
        ...
        throw new GeneralValidationException("Imagem aluno", 
  ...
  private void verificarSeVazio(byte[] arquivo) {
    if (arquivo.length == 0) {
      throw new GeneralValidationException("Imagem aluno", "Arquivo vazio");
    }
  }

  private void verificarTamanhoImagem(byte[] arquivo) {
    if (arquivo.length > MAX_FILE_SIZE)
      throw new GeneralValidationException("Imagem aluno", "Arquivo maior que 10mb");
  }
  ...
  private String verificarTipoImagem(byte[] arquivo) {
    try (InputStream is = new ByteArrayInputStream(arquivo)) {
      ...
      if (!SUPPORTED_MIME_TYPES.contains(mimeType))
        throw new GeneralValidationException("Imagem aluno", "Tipo de imagem não suportada.");
      return mimeType;
    } catch (IOException e) {
      throw new GeneralValidationException("Imagem aluno", "Erro ao verificar o tipo da imagem");
    }
  }
}
